...
# Custom callback to track test set performance without affecting training time
...
for hidden_nodes in HIDDEN_NODES_LIST:
    ...
    # Create callback to track test performance
    test_callback = TestEvaluationCallback((X_test, y_test))
    
    # Train model WITHOUT validation_data (for accurate Table 6 timing)
    ...
    history = model.fit(
        ...
        verbose=1,
        callbacks=[test_callback]  # Track test performance during training
    )
    train_time = time.time() - start_train
    
    # Plot training history using test set as validation
